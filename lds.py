...
def lds_inference(p_X, p_U, dynamics, smooth=False):
    ...
    def kalman_filter(previous, potential):
        ...
        J_tt = T.core.Print(J_tt,
                            [T.core.matrix_diag_part(T.matrix_inverse(potential[0]))[0],
                             T.matrix_solve(potential[0], potential[1][...,
                                                                       None])[...,
                                                                              0][0]],
                            'Current Potential:')
        ...
        J_tt = T.core.Print(J_tt,
                            [T.core.matrix_diag_part(T.matrix_inverse(J_t1_t))[0],
                             T.matrix_solve(J_t1_t, h_t1_t[..., None])[..., 0][0]],
                            'Next Potential:')
    ...
    if smooth:
        ...
        _, smoothed = T.scan(kalman_smooth, (filtered[0][:-1],
                                             filtered[1][:-1]),
                             (0,),
                             reverse=True)
    else:
